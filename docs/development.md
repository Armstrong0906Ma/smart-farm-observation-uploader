...
- `GET /api/plants` seeds default plants `A-1-1` through `D-3-5` if Firestore is empty.
- `POST /api/observations` stores the observation in Firestore only and sets `uploadStatus = pending`.
...
Cloud Tasks retry is not wired yet; manual sync is currently handled by the Next.js API route.
